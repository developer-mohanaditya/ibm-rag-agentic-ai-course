...
from langchain_community.embeddings import HuggingFaceEmbeddings  # LangChain embeddings wrapper
...
# Initialize LLM Setup using Mistral (OpenAI-compatible)
...
def initialize_llm(model_id, mistral_api_key):
    # Use Mistral's OpenAI-compatible endpoint with LangChain's ChatOpenAI
    return ChatOpenAI(
        model=model_id,
...
def summarize_video(video_url):
    ...
    if processed_transcript:
        ...
        # Step 2: Initialize Mistral LLM for summarization
        llm = initialize_llm(model_id, mistral_api_key)
