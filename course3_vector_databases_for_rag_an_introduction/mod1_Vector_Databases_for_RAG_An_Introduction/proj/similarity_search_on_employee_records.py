...
def main():
    try:
        ...
        # Defining a list of employee records
        employees = [
            {
                "id": "employee_1",
                "name": "John Doe",
        ...
        # Create comprehensive texts for each employee record
        ...
        for employee in employees:
            ...
            document += f"Employement type: {employee['employment_type']}."
            employee_documents.append(document)
            
        # Adding data to the collection in Chroma DB
        collection.add(
            
            # Employee documents to be added
            documents=employee_documents,
            
            # Metadata for each employee
            metadatas=[{
                "name": emp["name"],
        ...
        # Retrieving all items from the specific collection
        all_items = collection.get()
        print(f"Total items in the collection: {len(all_items['ids'])}")
        ...
        def perform_advanced_search(collection, all_items):
            try:
                ...
                for i, (doc_id, document, distance) in enumerate(zip(
                    ...
                    print(f"{i+1}. {metadata['name']} (ID: {doc_id}) - Distance: {distance:.4f}")
                ...
                print("\n1. Filtering by department (Engineering):")
                results = collection.get(
                    where={"department": "Engineering"}
                )
                print(f"Total Engineering employees found: {len(results['ids'])}")
                for i, doc_id in enumerate(results['ids']):
                    metadata = results['metadatas'][i]
                    print(f"  - {metadata['name']}: {metadata['role']} ({metadata['experience']} years.)")
                    
                # Example 2: Filter by experience range
                print("\n2. Filtering by experience range (10+ years):")
                results = collection.get(
                    where={"experience": {"$gte": 10}}
                )
                print(f"Total employees with 10+ years of experience: {len(results['ids'])}")
                for i, doc_id in enumerate(results['ids']):
                    metadata = results['metadatas'][i]
                    print(f"  - {metadata['name']}: {metadata['role']} ({metadata['experience']} years.)")
                    
                # Example 3: Filter by location
                print("\n3. Filtering by location in California:")
                results = collection.get(
                    where={"location": {"$in": ["San Francisco", "Los Angeles"]}}
                )
                print(f"Total employees in California: {len(results['ids'])}")
                for i, doc_id in enumerate(results['ids']):
                    metadata = results['metadatas'][i]
                    print(f"  - {metadata['name']}: {metadata['role']} ({metadata['location']})")
                
                
                print("\n=== Combined Search: Similarity + Metadata Filtering ===")
                
                # Example: Find experienced Python developers in specific locations
                print("\n6. Searching for senior Python Developers in major tech cities: ")
                ...
                print(f"Total results found: {len(results['ids'][0])}")
                for i, (doc_id, document, distance) in enumerate(zip(
                    results['ids'][0], results['documents'][0], results['distances'][0]
                )):
                    metadata = results['metadatas'][0][i]
                    print(f"  {i+1}. {metadata['name']} ({doc_id}) - Distance: {distance:.4f}")
                    print(f"     {metadata['role']} in {metadata['location']} with {metadata['experience']} years of experience.")    
                    print(f"     Document snippet: {document[:80]}...")
                
                # check if the results are empty or undefined
                if not results or not results['ids'] or len(results['ids'][0]) == 0:
                    print(f"No results found for the advanced search to {query_text}.")
                    return
                
                
                # Log the header for the top 3 similar documents based on the query
                print(f"\nTop 3 similar documents for the query '{query_text}':")
                ...
                print(f"An error occurred during advanced search: {error}")
        
        # Call the perform_advanced_search function with the colection and all_items
